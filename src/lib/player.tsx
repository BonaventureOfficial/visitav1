...
export function PlayerProvider({ children }: { children: ReactNode }) {
  ...
  return (
    <Ctx.Provider value={{
      ...
      // Mini-first: never auto-expand. User must tap maximize to go full screen.
...
function PersistentPlayer() {
  ...
  // Reset accumulator when video changes
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const onTime = async () => {
      ...
      // Only accumulate continuous playback (ignore seeks)
      ...
      if (
        ...
        user &&
        current.id &&
        !recordedRef.current.has(current.id)
      ) {
        recordedRef.current.add(current.id);
        const { error } = await (supabase as any)
          .from("video_views")
          .insert({ user_id: user.id, video_id: current.id });
        // 23505 = unique_violation → already viewed today, ignore silently
        if (error && (error as any).code !== "23505") {
          recordedRef.current.delete(current.id);
        }
      }
    };
  ...
  return (
    ...
    >
      <div
        className={expanded ? "flex-1 flex items-center justify-center bg-black relative" : "relative bg-black"}
        style={expanded ? undefined : { height: size.h }}
      >
        <video
          ref={videoRef}
...
function ChannelStrip({ video }: { video: PlayingVideo }) {
  ...
  useEffect(() => {
    supabase.from("videos").select("likes,views").eq("id", video.id).maybeSingle()
      .then(({ data }) => {
        if (data) { setLikeCount((data as any).likes ?? 0); setViewCount((data as any).views ?? 0); }
      });
    if (user) {
      (supabase as any).from("video_likes")
        .select("id").eq("user_id", user.id).eq("video_id", video.id).maybeSingle()
  ...
  const toggleLike = async () => {
    ...
    if (liked) {
      ...
      const { error } = await (supabase as any).from("video_likes")
  ...
  return (
    ...
    >
      ...
      <button
        onClick={toggleLike}
        className={`rounded-full h-10 w-10 flex items-center justify-center transition shrink-0 ${
          liked ? "bg-primary/15 text-primary border border-primary/50" : "bg-secondary text-foreground border border-border"
        }`}
        aria-label="Like"
      >
